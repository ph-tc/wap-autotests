window.onload = function() {
  ...
  checkFonts( detectFonts() );
	...
	// проверка изображений (в качестве бэкграунда)

	const allElems = document.querySelectorAll('body *');
	const bgImageElems = [];
	let bgImagesCheck = true;
	const srcImageElems = [];
	let srcImagesCheck = true;
	...
	for ( let i = 0; i < bgImageElems.length; i++ ) {
		if ( getComputedStyle(bgImageElems[i]).backgroundImage.endsWith('.png")') ||
			getComputedStyle(bgImageElems[i]).backgroundImage.endsWith('.jpg")') ||
			getComputedStyle(bgImageElems[i]).backgroundImage.endsWith('.jpeg")') ||
			getComputedStyle(bgImageElems[i]).backgroundImage.endsWith('.svg")') ) {
			console.error('Имеются внешние картинки (допускаются только инлайновые - в base64)');
			bgImagesCheck = false;
			break;
		} else if ( getComputedStyle(bgImageElems[i], ':after').backgroundImage.endsWith('.png")') ||
			getComputedStyle(bgImageElems[i], ':after').backgroundImage.endsWith('.jpg")') ||
			getComputedStyle(bgImageElems[i], ':after').backgroundImage.endsWith('.jpeg")') ||
			getComputedStyle(bgImageElems[i]).backgroundImage.endsWith('.svg")') ) {
			console.error('Имеются внешние картинки (допускаются только инлайновые - в base64)');
			bgImagesCheck = false;
			break;
		} else if ( getComputedStyle(bgImageElems[i], ':before').backgroundImage.endsWith('.png")') ||
			getComputedStyle(bgImageElems[i], ':before').backgroundImage.endsWith('.jpg")') ||
			getComputedStyle(bgImageElems[i], ':before').backgroundImage.endsWith('.jpeg")') ||
			getComputedStyle(bgImageElems[i]).backgroundImage.endsWith('.svg")') ) {
			console.error('Имеются внешние картинки (допускаются только инлайновые - в base64)');
			bgImagesCheck = false;
			break;
	...
	// проверка изображений в качестве src
	for ( let i = 0; i < allElems.length; i++ ) {
		if ( allElems[i].src && (allElems[i].src.endsWith('.png') ||
			allElems[i].src.endsWith('.jpg') ||
			allElems[i].src.endsWith('.jpeg')||
			allElems[i].src.endsWith('.svg')) ) {
			console.error('Имеются внешние картинки (допускаются только инлайновые - в base64)');
			srcImagesCheck = false;
		}
	}

	if ( bgImagesCheck && srcImagesCheck ) {
  ...
  if ( document.getElementById('activation-btn') ) {
    ...
    if ( btnText ) {
      ...
      if ( btnIconMargin >= 7 ) {
        ...
        console.error(`Расстояние иконки до текста меньше 7px (или отсутствует иконка)`);
      }
    } else {
      console.error(`Отсутствует текст на кнопке`);
    }
    ...
    if ( btnText ) {
      ...
      if ( btnTextStyles.transform !== 'translate(-50%, -50%)' &&
        ...
        btnTextStyles.left !== '50%' || btnTextStyles.textAlign !== 'center' ) {
  ...
  if (document.querySelector('.info-block-text .price .line2')) {
    ...
    if ( priceFontSize >= 22 ) {
      ...
      console.error(`Размер шрифта цены меньше 22px`);
		...
		const btnCoordsBottom = btn.getBoundingClientRect().bottom.toFixed(0);
		const priceCoordsTop = price.getBoundingClientRect().top.toFixed(0);

    if ( priceCoordsTop - btnCoordsBottom ) {
      console.log(`Расстояние от цены до кнопки - ок ( ${priceCoordsTop - btnCoordsBottom}px )`);
    } else {
    ...
    let priceBg;
    let cond = false;
    let priceForLoop = price;

    while ( !cond ) {
			priceBg = getComputedStyle(priceForLoop).backgroundColor;

			priceBg !== 'rgba(0, 0, 0, 0)' || priceForLoop.classList.contains('info-block-wrapper') ? cond = !cond : priceForLoop = priceForLoop.parentElement;
		}

    if ( priceBg && priceBg !== 'rgba(0, 0, 0, 0)' ) {
  ...
  console.info(`%cПроверь закрытые одиночные теги\n<meta/> <link/> <br/> <img/> <input/>!`, `display:block;background-color:darkcyan;color:white`);
